function onEvent(_) {
    if (_.event.name == 'Tween Camera Zoom') {
        ...
        camZooming = false;
        FlxTween.tween(FlxG.camera, {zoom: _.event.params[0]}, finalTime, {ease: eventEase, onUpdate: function(value) {defaultCamZoom = FlxG.camera.zoom;}, onComplete: function(tween:FlxTween) {
        	camZooming = true;
        }});
    }
}